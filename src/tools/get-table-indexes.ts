...
interface GetTableIndexesArgs {
  tableName: string;
  includeStats?: boolean;
}

export async function getTableIndexesHandler(args: GetTableIndexesArgs) {
  if (!args?.tableName) {
  ...
  return withConnection(async (connection) => {
    ...
    let exprRows: any[] = [];
    try {
      const exprResult = await connection.execute(
        exprSql,
        { tableName: args.tableName },
        { outFormat: oracledb.OUT_FORMAT_OBJECT }
      );
      exprRows = (exprResult.rows as any[]) || [];
    } catch {
    ...
    const constraintMap: Record<string, { name: string; type: string }> = {};
    if (constraintResult.rows) {
      for (const row of constraintResult.rows as any[]) {
    ...
    if (columnResult.rows) {
      for (const row of columnResult.rows as any[]) {
        if (!columnMap[row.INDEX_NAME]) columnMap[row.INDEX_NAME] = [];
    ...
    for (const row of exprRows) {
      if (!exprMap[row.INDEX_NAME]) exprMap[row.INDEX_NAME] = [];
    ...
    const indexes = (indexResult.rows as any[]).map((idx) => {
