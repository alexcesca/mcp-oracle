...
export const validatePackageStandardsDefinition = {
    ...
    description: "Valida se um pacote Oracle segue os padrões de desenvolvimento SIGA definidos em Padrao_pl_sql.md. " +
...
function splitSpecBody(rows) {
    const spec = [];
    const body = [];
    for (const r of rows) {
        if (r.TYPE === "PACKAGE")
            spec.push(r);
        else if (r.TYPE === "PACKAGE BODY")
            body.push(r);
    }
    return { spec, body };
}
...
export async function validatePackageStandardsHandler(args) {
    ...
    `;
        ...
        for (const r of allRows) {
            if (r.TEXT.includes("\t"))
                tabLines.push(r.LINE);
        }
        if (tabLines.length > 0) {
            fails.push(`[FAIL] TAB characters found on ${tabLines.length} line(s). ` +
        ...
        if (body.length > 0) {
            ...
            for (let i = 0; i < bodyLines.length; i++) {
                ...
                for (let j = i - 1; j >= 0 && j >= i - 40; j--) {
                    ...
                    if (/^\s*(procedure|function|begin|end)\b/i.test(bodyLines[j]))
                        break;
                }
                if (!foundComment) {
                    fails.push(`[FAIL] ${kind === "procedure" ? "Procedure" : "Function"} '${name}' ` +
                    ...
                    for (const field of headerRequiredFields) {
                        if (!commentBlock.toLowerCase().includes(field.toLowerCase())) {
                            warns.push(`[WARN] Header comment for '${name}' ${lineRef(body[i].LINE)} ` +
                                `is missing required field '${field}:'.`);
                        }
                    }
                    // Functions must also have "Retorno:"
                    if (kind === "function" && !commentBlock.toLowerCase().includes("retorno")) {
                        warns.push(`[WARN] Function '${name}' ${lineRef(body[i].LINE)} header is missing 'Retorno:' field.`);
                    }
                }
            }
        }
        ...
        subprogramMatches.forEach((match) => {
            ...
            if (keyword === "procedure") {
                ...
                if (!name.startsWith("PKB_") && !name.startsWith("PKL_")) {
                    fails.push(`[FAIL] Procedure '${name}' does not follow naming convention. ` +
            ...
            if (keyword === "function") {
                ...
                if (!name.startsWith("FKG_") && !name.startsWith("FKL_")) {
                    fails.push(`[FAIL] Function '${name}' does not follow naming convention. ` +
        ...
        for (const name of procNames) {
            ...
            if (opPart && !validOperations.includes(opPart)) {
                infos.push(`[INFO] Procedure '${name}' uses non-standard operation identifier '${opPart}'. ` +
        ...
        for (const name of allNames) {
            ...
            if (fullName.length > 30) {
                warns.push(`[WARN] '${fullName}' has ${fullName.length} characters (max 30). ` +
        ...
        const paramRegex = /(\w+)\s+(?:in\s+out\s+nocopy|in\s+out|out\s+nocopy|out|in)\s+/gi;
        let paramMatch;
        while ((paramMatch = paramRegex.exec(fullSource)) !== null) {
            const paramName = paramMatch[1].toLowerCase();
            // skip keywords that might match
            if (["procedure", "function", "is", "as", "return", "begin", "end"].includes(paramName))
                continue;
            ...
            if (isInOut) {
                if (!validInOutPrefixes.some((p) => paramName.startsWith(p))) {
                    warns.push(`[WARN] IN OUT parameter '${paramName}' should use prefix esn_/esv_/esd_/esb_/est_/esr_/esc_.`);
                }
            }
            else if (isOut) {
                if (!validOutputPrefixes.some((p) => paramName.startsWith(p))) {
                    warns.push(`[WARN] OUT parameter '${paramName}' should use prefix sn_/sv_/sd_/sb_/st_/sr_/sc_.`);
                }
            }
            else if (isIn) {
                if (!validInputPrefixes.some((p) => paramName.startsWith(p))) {
                    warns.push(`[WARN] IN parameter '${paramName}' should use prefix en_/ev_/ed_/eb_/et_/er_/ec_.`);
                }
            }
        }
        ...
        while ((excMatch = exceptionDeclRegex.exec(fullSource)) !== null) {
            ...
            if (!excName.startsWith("ve_")) {
                fails.push(`[FAIL] Exception variable '${excName}' must start with 've_' prefix.`);
            }
        }
        ...
        while ((curMatch = cursorDeclRegex.exec(fullSource)) !== null) {
            ...
            if (!curName.startsWith("cur_")) {
                warns.push(`[WARN] Cursor variable '${curName}' should start with 'cur_' prefix.`);
            }
        }
        // ════════════════════════════════════════════════════════════════════════════
        ...
        while ((typeMatch = typeRecordRegex.exec(fullSource)) !== null) {
            ...
            if (!typeName.startsWith("t_rec_")) {
                fails.push(`[FAIL] Record type '${typeName}' must use prefix 't_rec_'.`);
            }
        }
        ...
        while ((typeMatch = typeTableRegex.exec(fullSource)) !== null) {
            ...
            if (!typeName.startsWith("t_tab_rec_")) {
                warns.push(`[WARN] Table/collection type '${typeName}' should use prefix 't_tab_rec_'.`);
            }
        }
        // ════════════════════════════════════════════════════════════════════════════
        ...
        if (totalMissing > 0) {
            warns.push(`[WARN] Found ${totalMissing} OUT/IN OUT parameter(s) possibly missing 'NOCOPY' directive. ` +
        ...
        while ((constMatch = constantRegex.exec(fullSource)) !== null) {
            ...
            if (constName !== constName.toUpperCase()) {
                fails.push(`[FAIL] Constant '${constName}' must be declared in UPPERCASE.`);
            }
        }
        // ════════════════════════════════════════════════════════════════════════════
        ...
        if (lowercaseDdlFound.length > 0) {
            warns.push(`[WARN] DDL keywords found in lowercase: ${lowercaseDdlFound.join(", ")}. ` +
        ...
        if (selectStarMatches.length > 0) {
            fails.push(`[FAIL] Found ${selectStarMatches.length} occurrence(s) of 'SELECT *'. ` +
        ...
        if (dbmsMatches.length > 0) {
            fails.push(`[FAIL] Found ${dbmsMatches.length} occurrence(s) of DBMS_OUTPUT. ` +
        ...
        const commentedCodeRegex = /--\s*(select|insert|update|delete|begin|end|if|else|for|while|loop|declare|procedure|function|exception)\b/gi;
        const commentedCodeMatches = fullSource.match(commentedCodeRegex) || [];
        if (commentedCodeMatches.length > 3) {
            warns.push(`[WARN] Found ${commentedCodeMatches.length} lines of potentially commented-out code. ` +
        ...
        if (rowtypeMatches.length > 0) {
            if (isScreenPackage) {
                fails.push(`[FAIL] Found ${rowtypeMatches.length} usage(s) of %ROWTYPE. ` +
                    `%ROWTYPE is PROHIBITED in screen/program packages. Declare types field-by-field.`);
            }
            else {
                warns.push(`[WARN] Found ${rowtypeMatches.length} usage(s) of %ROWTYPE. ` +
        ...
        if (body.length > 0) {
            ...
            while ((funcBodyMatch = funcBodyRegex.exec(bodyFullText)) !== null) {
                ...
                if (/\b(insert\s+into|update\s+\w+\s+set|delete\s+from)\b/i.test(funcContent)) {
                    fails.push(`[FAIL] Function '${funcName}' contains DML statements (INSERT/UPDATE/DELETE). ` +
        ...
        if (isTablePackage) {
            ...
            for (const fn of mandatoryFunctions) {
                if (!allSubNames.includes(fn)) {
                    warns.push(`[WARN] Table package is missing mandatory function '${fn}'. ` +
                        `Every table package should include FKG_SEQ.`);
                }
            }
            for (const proc of mandatoryProcedures) {
                if (!allSubNames.includes(proc)) {
                    warns.push(`[WARN] Table package is missing mandatory procedure '${proc}'. ` +
                        `Every table package should include PKB_VAL.`);
                }
            }
            for (const proc of mandatoryCrudProcedures) {
                if (!allSubNames.includes(proc)) {
                    warns.push(`[WARN] Table package is missing standard CRUD procedure '${proc}'.`);
                }
            }
        }
        // ════════════════════════════════════════════════════════════════════════════
        ...
        if (isScreenPackage) {
            ...
            for (const proc of screenMandatory) {
                if (!allSubNames.includes(proc)) {
                    infos.push(`[INFO] Screen package may be missing standard procedure '${proc}' ` +
        ...
        if (body.length > 0) {
            ...
            if (!/when\s+others\s+then/i.test(bodyText)) {
                warns.push(`[WARN] No 'WHEN OTHERS THEN' exception handler found in package body. ` +
            ...
            if (/when\s+others\s+then/i.test(bodyText) && !/pk_soft\.pkb_busca_msg_erro/i.test(bodyText)) {
                warns.push(`[WARN] WHEN OTHERS handler(s) found but no call to 'pk_soft.pkb_busca_msg_erro'. ` +
            ...
            if (!/\bve_erro\b/i.test(bodyText)) {
                infos.push(`[INFO] No 've_erro' exception variable found. ` +
            ...
            for (const param of stdErrorParams) {
                if (!fullSource.toLowerCase().includes(param)) {
                    infos.push(`[INFO] Standard error output parameter '${param}' not found. ` +
        ...
        if (spec.length > 0) {
            ...
            for (const r of spec) {
                ...
                if (/\bconstant\b/.test(txt)) {
                    if (lastSection > 1) {
                        warns.push(`[WARN] Package Specification order violation ${lineRef(r.LINE)}: ` +
        ...
        if (body.length > 0) {
            ...
            if (lastFuncLine > firstProcLine && firstProcLine < Infinity) {
                infos.push(`[INFO] Package Body order: a Function is declared after the first Procedure ` +
        ...
        if (!/^PK_\w+$/i.test(packageName)) {
            warns.push(`[WARN] Package name '${packageName}' does not follow the naming convention PK_[module]_[shortname] ` +
        ...
        if (/\bbulk\s+collect\b/i.test(bodyText) && !/\blimit\b/i.test(bodyText)) {
            infos.push(`[INFO] BULK COLLECT found without LIMIT clause. For large datasets (>50,000 rows), ` +
        ...
        if (/nvl\s*\(\s*\w+\.count/i.test(fullSource)) {
            infos.push(`[INFO] nvl() used with .count — COUNT always returns a value and never NULL. ` +
        ...
        if (isScreenPackage && /pk_sfpadrao\./i.test(bodyText)) {
            ...
            const sfpadraoProcContext = bodyText.match(/procedure\s+(pkb_(?!valida\b|postquer\b)\w+)[\s\S]*?pk_sfpadrao\./gi);
            if (sfpadraoProcContext && sfpadraoProcContext.length > 0) {
                fails.push(`[FAIL] pk_sfpadrao usage detected outside PKB_VALIDA/PKB_POSTQUER. ` +
        ...
        if (/\btype\s+\w+\s+is\s+object\b/i.test(fullSource)) {
            fails.push(`[FAIL] TYPE OBJECT found. SIGA standard only allows TYPE RECORD. ` +
        ...
        const parseFinding = (rawMsg, severity) => {
            ...
            if (!recommendation)
                recommendation = "Revise o código para atender os padrões SIGA.";
            return {
                severity,
