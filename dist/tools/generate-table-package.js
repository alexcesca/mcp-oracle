...
export const generateTablePackageDefinition = {
    ...
    description: "Gera um pacote Oracle PL/SQL completo compatível com o padrão SIGA (spec .pks + body .pkb) para uma determinada tabela. " +
    ...
    inputSchema: {
        ...
        properties: {
            ...
            moduleName: {
                type: "string",
                description: "Prefixo de abreviação do módulo (ex: 'AED', 'EFD', 'COR'). Usado para nomear o pacote: PK_[MODULO]_[NOME_CURTO]. Se omitido, o módulo é inferido ou deixado em branco.",
            },
            shortName: {
                type: "string",
                description: "Nome curto personalizado opcional para a tabela (ex: 'BEMIMOBEFD'). Se omitido, será derivado automaticamente do nome da tabela.",
            },
            author: {
                type: "string",
...
// ─── Helpers ─────────────────────────────────────────────────────────────────
function deriveShortName(tableName) {
...
function getPrefixForType(colName, dataType) {
    const name = colName.toLowerCase();
    if (dataType === "DATE" || dataType === "TIMESTAMP")
        return { param_in: "ed_", param_out: "sd_", local: "vd_" };
    if (dataType === "CLOB" || dataType === "BLOB")
        return { param_in: "ec_", param_out: "sc_", local: "vc_" };
    if (dataType === "NUMBER")
        return { param_in: "en_", param_out: "sn_", local: "vn_" };
    return { param_in: "ev_", param_out: "sv_", local: "vv_" };
}
function isPkColumn(col, pkColumns) {
...
function header(definition, params, returnDesc) {
    const lines = [
        "/*",
        SEPARATOR,
...
// ─── Code Generators ─────────────────────────────────────────────────────────
function generateSpec(tableName, packageName, shortName, columns, pkColumns, seqName, domains, author) {
    ...
    if (domains.length > 0) {
        ...
        for (const d of domains) {
            if (!grouped[d.column_name])
                grouped[d.column_name] = [];
    ...
    const validaParams = ukCols.map(c => {
        const { param_in } = getPrefixForType(c.column_name, c.data_type);
...
function emitProc(name, params, lines, tbl, sn, _desc) {
...
function generateBody(tableName, packageName, shortName, columns, pkColumns, seqName, author) {
    const tbl = tableName.toLowerCase();
    const sn = shortName.toLowerCase();
    const PKG = packageName.toUpperCase();
    const tabType = `t_tab_rec_${sn}`;
    ...
    }).concat([{ name: `est_${sn}`, desc: "Registros encontrados" }]), conBody, PKG, `PKB_CON_${shortName.toUpperCase()}`, lines, author, dt, true);
    L();
    // ── PKB_INC ──
    const incBody = buildInsert(tbl, columns, pkColumns, seqName, shortName, PKG);
    emitBodyProc("PKB_INC", `Inclusão de registros na tabela ${tbl}`, [{ name: `est_${sn}`, desc: "Registros a inserir" }], incBody, PKG, `PKB_INC_${shortName.toUpperCase()}`, lines, author, dt, true);
    L();
    // ── PKB_ALT ──
    const altBody = buildUpdate(tbl, columns, pkColumns, shortName, PKG);
    emitBodyProc("PKB_ALT", `Alteração de registros na tabela ${tbl}`, [{ name: `est_${sn}`, desc: "Registros a alterar" }], altBody, PKG, `PKB_ALT_${shortName.toUpperCase()}`, lines, author, dt, true);
    L();
    // ── PKB_EXC ──
    emitBodyProc("PKB_EXC", `Exclusão de registros da tabela ${tbl}`, [{ name: `est_${sn}`, desc: "Registros a excluir (deve conter o ID)" }], [
        `      begin`,
        `         delete from ${tbl}`,
    ...
    L();
    // ── PKB_LOC ──
    emitBodyProc("PKB_LOC", `Lock de registros da tabela ${tbl}`, [{ name: `est_${sn}`, desc: "Registro a bloquear" }], [
        `      begin`,
        `         select ${idCol}`,
    ...
    emitBodyProc("PKB_VALIDA", `Busca por Unique Key - retorna 1 registro`, [...ukParams, { name: `st_${sn}`, desc: "Registro encontrado (posição 1)" }], validaBody, PKG, "PKB_VALIDA", lines, author, dt, true);
    L();
    // ── PKB_POSTQUER ──
    emitBodyProc("PKB_POSTQUER", `Busca por ID - retorna 1 registro`, [{ name: `en_${idCol}`, desc: "ID do registro" }, { name: `st_${sn}`, desc: "Registro encontrado (posição 1)" }], buildPostquer(tbl, columns, idCol, shortName), PKG, "PKB_POSTQUER", lines, author, dt, true);
    L();
    // ── PKB_LOV ──
    const descrCol = columns.find(c => c.column_name.toLowerCase().includes("descr"))
        ...
        || columns[1];
    emitBodyProc("PKB_LOV", `Lista de valores ordenada`, [{ name: `st_${sn}`, desc: "Registros encontrados" }], buildLov(tbl, columns, descrCol?.column_name.toLowerCase() || "cd", shortName), PKG, "PKB_LOV", lines, author, dt, true);
    L();
    L(`END ${PKG};`);
    return lines.join("\n");
}
function emitBodyProc(name, definition, params, bodyLines, pkg, qualifiedName, lines, author, dt, hasVarSection = false) {
    lines.push(`   /*`);
    lines.push(`   ${SEPARATOR}`);
...
function buildInsert(tbl, columns, pkColumns, seqName, shortName, pkg) {
    const sn = shortName.toLowerCase();
    const idCol = pkColumns[0]?.toLowerCase() || `${sn}_id`;
    const lines = [];
    lines.push(`      begin`);
...
function buildUpdate(tbl, columns, pkColumns, shortName, pkg) {
    const sn = shortName.toLowerCase();
    const idCol = pkColumns[0]?.toLowerCase() || `${sn}_id`;
...
function buildValida(tbl, columns, pkColumns, ukCols, shortName) {
    const sn = shortName.toLowerCase();
    const lines = [];
    lines.push(`   BEGIN`);
    lines.push(``);
...
function buildPostquer(tbl, columns, idCol, shortName) {
    const sn = shortName.toLowerCase();
    const lines = [];
    lines.push(`   BEGIN`);
    lines.push(``);
...
function buildLov(tbl, columns, orderCol, shortName) {
    const sn = shortName.toLowerCase();
    const lines = [];
    lines.push(`   BEGIN`);
    lines.push(``);
...
export async function generateTablePackageHandler(args) {
    ...
    return withConnection(async (connection) => {
        // 1. Fetch columns
        const colResult = await connection.execute(`SELECT column_name, data_type, data_length, data_precision, data_scale, nullable, column_id
       FROM user_tab_columns
       WHERE table_name = :tableName
       ORDER BY column_id`, { tableName }, { outFormat: oracledb.OUT_FORMAT_OBJECT });
        ...
        const pkResult = await connection.execute(`SELECT cc.column_name
       FROM user_constraints c
       JOIN user_cons_columns cc ON c.constraint_name = cc.constraint_name
       WHERE c.table_name = :tableName AND c.constraint_type = 'P'
       ORDER BY cc.position`, { tableName }, { outFormat: oracledb.OUT_FORMAT_OBJECT });
        const pkColumns = (pkResult.rows || []).map((r) => r.COLUMN_NAME.toUpperCase());
        // 3. Find sequence
        const pkCol = pkColumns[0]?.toLowerCase();
        let seqName = null;
        if (pkCol) {
            const seqResult = await connection.execute(`SELECT sequence_name FROM user_sequences
         WHERE sequence_name LIKE :pattern
         ORDER BY sequence_name`, { pattern: `${tableName.replace(/_TV$/, "")}_SEQ%` }, { outFormat: oracledb.OUT_FORMAT_OBJECT });
            seqName = seqResult.rows?.[0]?.SEQUENCE_NAME?.toLowerCase() || null;
        }
        // 4. Fetch domain constants
        let domains = [];
        try {
            const domResult = await connection.execute(`SELECT rv_meaning, rv_low_value, rv_domain,
                substr(rv_domain, instr(rv_domain, '.') + 1) as column_name
         FROM cg_ref_codes
         WHERE rv_domain LIKE :domain
         ORDER BY rv_domain, rv_low_value`, { domain: `${tableName}.%` }, { outFormat: oracledb.OUT_FORMAT_OBJECT });
            domains = (domResult.rows || []).map((r) => ({
