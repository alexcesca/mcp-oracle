...
if (!ORACLE_USER || !ORACLE_PASSWORD || !ORACLE_CONNECT_STRING) {
    console.error("Missing required environment variables: ORACLE_USER, ORACLE_PASSWORD, ORACLE_CONNECT_STRING");
    process.exit(1);
}
...
async function runHttp() {
    const httpServer = http.createServer(async (req, res) => {
        ...
        if (req.url === "/mcp" || req.url?.startsWith("/mcp?")) {
            ...
            const isBrowserGet = req.method === "GET" && !req.headers.accept?.includes("text/event-stream");
            if (isBrowserGet) {
                const info = {
                    name: "mcp-oracle",
                    version: "1.0.0",
